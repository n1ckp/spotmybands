...
import {
  actionFetchArtistEvents,
  actionToggleArtistEvents,
} from "@utils/globalState/events";
...
const ArtistRow: React.FC<ArtistRowProps> = ({
  ...
  const eventsShown =
    ...
    !artistData.hidden &&
    !artistData.loading &&
    artistData.events.length;
  const noEvents =
    artistData && !artistData.loading && artistData.events.length === 0;
  ...
  if (!onUserList) {
    button = (
      <Button onClick={() => addToUserList(artist)}>Add to my list</Button>
    );
  } else if (!eventsFetched) {
    button = (
      <Button type="primary" onClick={() => fetchArtistEvents()}>
        Fetch Events
      </Button>
    );
    ...
    button = (
      <Button
        type={eventsShown ? undefined : "primary"}
        onClick={() => toggleEvents(eventsShown)}
      >
  ...
  if (onUserList) {
    removeButton = (
      <Button className={styles.delete} onClick={() => onRemoveArtist()}>
        <DeleteIcon />
      </Button>
    );
  }
    ...
    <LoadingOverlay active={fetchingEvents}>
      <div id={styles.container} className={className}>
        <span
          ...
          style={{ backgroundImage: `url(${artist.logoURL})` }}
        ></span>
        <div className={styles.info}>
          <h2>{artist.name}</h2>
          {!onUserList && artist.genres && (
